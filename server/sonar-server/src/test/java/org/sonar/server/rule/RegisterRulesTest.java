...
public class RegisterRulesTest {
  ...
  public void rules_that_deprecate_previous_rule_must_be_recorded() {
    ...
    execute(context -> {
      ...
      repo.createRule("newKey")
        ...
        // This deprecated key won't be stored
        .addDeprecatedRuleKey("not_created", "rule1");
      repo.done();
    });

    List<RuleDefinitionDto> rules = dbClient.ruleDao().selectAllDefinitions(dbTester.getSession());
    List<DeprecatedRuleKeyDto> deprecatedRuleKeys = dbClient.ruleDao().selectAllDeprecatedRuleKeys(dbTester.getSession());
    //assertThat(rules).hasSize(1); FIXME this must be true when renaming is done
    assertThat(deprecatedRuleKeys).hasSize(1);
  }
  ...
  @Test
  public void new_rules_with_absent_deprecated_keys_will_not_create_records() {
    execute(context -> {
      NewRepository repo = context.createRepository("fake", "java");
      repo.createRule("rule1")
        .setName("One")
        ...
        .setType(RuleType.CODE_SMELL)
        .setStatus(RuleStatus.BETA)
        .addDeprecatedRuleKey("squid", "rule2")
        .addDeprecatedRuleKey("squid", "rule3");
      repo.done();
    });
